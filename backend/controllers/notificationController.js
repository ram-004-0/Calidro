...
const generateDailyReminders = async () => {
  console.log("⏰ DEBUG: Starting Notification Logic...");
  try {
    // 1. Get ANY confirmed booking just to test the connection
    const [allBookings] = await query(
      "SELECT * FROM booking WHERE status = 'confirmed' LIMIT 1",
    );

    if (allBookings.length === 0) {
      return {
        success: false,
        message: "No confirmed bookings found in DB to test with.",
      };
    }

    const testBooking = allBookings[0];

    // 2. Force insert a test notification
    await query(
      "INSERT INTO notifications (user_id, message, type) VALUES (?, ?, ?)",
      [
        testBooking.user_id,
        `SYSTEM TEST: Testing notification logic at ${new Date().toLocaleTimeString()}`,
        "reminder",
      ],
    );
