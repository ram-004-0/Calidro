...
export default function BookingPage({ onNext }) {
  ...
  const unavailableDates = ["2026-04-05", "2026-04-06", "2026-04-15"];
  ...
  const getDayStyle = (date) => {
    ...
    if (isUnavailable(date)) return "bg-red-300";
  ...
  const handleNextClick = () => {
    ...
    onNext({
      ...
      time: convertTo24Hour(selectedTime), // <--- CONVERTED HERE
  ...
  return (
    <div className="bg-[#f1f1f1] w-full max-w-7xl rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-10">
      ...
      {/* CALENDAR PANEL */}
      <div className="bg-white rounded-2xl shadow-inner p-6 flex flex-col justify-between max-w-190 shadow-md">
        <div>
          ...
          <div className="grid grid-cols-7 text-center font-bold text-xs text-gray-400 mb-4 uppercase tracking-widest">
          ...
          <div className="grid grid-cols-7 gap-2 text-center">
            {days.map((day) => (
              <div
                ...
                className={`py-3 rounded-xl cursor-pointer text-sm font-semibold transition ${getDayStyle(day)}`}
              >
                {format(day, "d")}
              </div>
            ))}
        ...
        <div className="flex justify-center gap-6 mt-8 pt-6 border-t border-gray-100 text-xs font-medium">
